...
typedef unsigned L; typedef struct{I c;L bl;} Wat;typedef unsigned char u8;
/* ----------------USER INTERFACE----------------- */
#define or(a...) {int r[]={a};usr_clause(arrlen(r),r);}
I usr_clause(I,I*),solve(V);V dump(V),reset(V);I L2I(I);L I2L(I);
I attempts=1e6; F decay=0.99; u64 seed=12346;
enum{_=-1u/2, VARS=(I)1e5, LITS=VARS*2+1, GC_SZ=(I)1e6, GC_PERM=GC_SZ/4};
#define N I n         /*                                                 */
...
#define ca (gC.a+c+1) /*  gV: cause, level, activity, value, seen        */
#define i(a,e...)   ({I i=0,_end_=a;W(i<_end_){e;i++;}_end_!=i;})
#define k(a,b,e...) ({I k=a,_end_=b;W(k<_end_){e;k++;}_end_!=k;})
#define r(a,e...) ({typeof(a)r=a;e;r;}) /* ({ GNU C statement expression }) */
#define arrlen(a) (I)(sizeof(a)/sizeof*(a))
#define AS(a) assert(a);
#define p(b,v) r(g##b.n++,AS(g##b.n<arrlen(g##b.a));g##b.a[r]=v)
...
struct{N;Wat*a;} gW[LITS]; // c=clause, bl=blocker. TODO: dont use malloc?
...
struct{N,c[VARS],lv[VARS];F a[VARS];u8 v[VARS];u64 s[VARS/64];} gV;
V reset(V){i(gV.n+1,gV.v[i]=gV.c[i]=gV.lv[i]=gV.ai=0);
...
Z V see(N){n/=2;gV.s[n/64]|=1ull<<n%64;}Z I seen(N){n/=2;R 1&gV.s[n/64]>>n%64;}
Z V unsee(N){n/=2;gV.s[n/64]&=~(1ull<<n%64);}Z I av(L v){N=gV.v[v/2];R n/2&(n^~v);}
Z V assign(L v){AS(gV.v[v/2]==0);gV.v[v/2]=2|v%2;}
...
Z V hp_put(I i){if(gH.b[i])R;gH.b[i]=gH.n;hp_up(p(H,i));}
Z I hp_pop(V){I*a=gH.a;R gH.n?r(*a,gH.b[*a=a[--gH.n]]=1;gH.b[r]=0;if(gH.n>1)hp_down(0)):0;}
Z V enq(L v,I c){assign(v);gV.c[v/2]=c;gV.lv[v/2]=gLv.n;p(Q,v);}
...
Z V watch(I c){c02(watch1(c,v,b))}Z I locked(I c){R av(*ca)&&gV.c[*ca/2]==c;}
...
Z I clause(I c,I usr){AS(usr||c+(I)cn<gC.temp);AS(cn);if(cn==1){enq(*ca,_);gC.n=c;R _;}
  if(!usr&&cn>8)c=r(make_space(cn),i(cn+1,gC.a[r+i]=gC.a[c+i]);gC.n=c);watch(c);R c;}
I usr_clause(N,int*a){R r(p(C,n),i(n,L v=I2L(ai);AS(v<LITS);
  p(C,v);v/=2;gV.n=(L)gV.n>v?gV.n:v;hp_put(v));clause(r,1));}
Z V back_to(I lv){if(lv>=gLv.n)R;I i;for(i=gQ.n-1;i>=gLv.a[lv];i--)
  {L v=gQ.ai/2;gV.v[v]=0;hp_put(v);}gQ.n=gQ.head=i+1; gLv.n=lv;}
Z L branch(V){L v;do{if(gH.n==0)R 0;v=hp_pop();}W(gV.v[v]);R v*2|rng()<_/2;}
Z V vbump(I i){gV.ai++;if(gH.b[i])hp_up(gH.b[i]-1);}Z V vdecay(V){i(gV.n,gV.ai*=decay);}
Z I prop(V){W(gQ.head<gQ.n){L v=gQ.a[gQ.head++];
 if(!wv.a)continue;Wat*i,*j,*end=wv.a+wv.n;
 for(i=j=wv.a;i<end;){if(i<end-2)__builtin_prefetch(gC.a+i[2].c);
  if(i->c==_){i++;continue;}if(av(i->bl)){*j++=*i++;continue;}
  I c=i->c;L*a=ca,n=v^1;if(*a==n){*a=a[1];a[1]=n;};AS(a[1]==n);
  if(*a!=(*i++).bl&&av(*a)){*j++=(Wat){c,*a};continue;}
  if(k(2,a[-1],if(!av(ak^1)){a[1]=ak;ak=n;watch1(c,a[1]^1,*a);B}))continue;
  *j++=(Wat){c,*a};if(av(*a^1)){W(i<end)*j++=*i++;wv.n-=i-j;R c;}
  enq(*a,c);}wv.n-=i-j;}R _;}
Z I analyze(I c,I*lv){I r=p(C,0);p(C,0);I deps=0,i=gQ.n;L v=0;
 do{AS(c!=_);AS(!v||v==*ca);k(!!v,cn,L v=ca[k];if(!seen(v)&&gV.lv[v/2]>0){
   vbump(v/2);see(v);if(gV.lv[v/2]<gLv.n)p(C,v);else deps++;});
  W(v=gQ.a[--i],!seen(v)){}c=gV.c[v/2];unsee(v);}W(--deps>0);
 c=r;cn=gC.n-r-1;L*a=ca;*a=v^1;k(i=1,cn,I c=gV.c[ak/2];if(c==_)a[i++]=ak;else
  {for(L j=1;j<cn;j++){v=ca[j];if(!seen(v)&&gV.lv[v/2]>0){a[i++]=ak;B}}});cn=i;
 if(cn==1)*lv=0;else{i=1;k(2,cn,if(gV.lv[ak/2]>gV.lv[ai/2])i=k);
  *lv=gV.lv[ai/2];r(a[1],a[1]=ai;ai=r);}i(gV.n/64+1,gV.s[i]=0);R c;}
I solve(V){init();I confs=999,RST=1000;i(attempts,I conf=prop();if(conf!=_){
 if(gLv.n==0)R 0;I lv=0,c=analyze(conf,&lv);back_to(lv);c=clause(c,0);
 if(c!=_)enq(*ca,c);vdecay();if(++confs>RST){RST*=1.1;confs=0;
 i(gV.n,gV.ai=rng()/(_/15.));for(I i=gH.n/2-1;i>=0;)hp_down(i--);back_to(0);}
 }else{L v=branch();if(!v)R 1;p(Lv,gQ.n);enq(v,_);});back_to(0);R _;}
V dump(V){
    pf("<Heap>\n")i(gH.n,pf("%d ",gH.ai));
    ...
    pf("\n<Assigns>")k(1,gV.n,pf("\n%c%d |%.1f",av(k*2)?' ':av(k*2^1)?'-':'?',k,gV.ak));
    pf("\n----------END DUMP----------\n")}
